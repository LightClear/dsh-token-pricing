...
import type { TokenPricingStep, TokenPricingTurn } from './types.ts'
 ...
 */
function usageStep(event: SessionEvent<'assistant/message'>): TokenPricingStep | null {
...
ProjectionDefinition<'tokenPricing', TokenPricingState> = {
  key: 'tokenPricing',
  ...
  apply: (state, event) => {
    switch (event.type) {
      ...
      case 'turn/start': {
        ...
        if (last !== undefined && last.turn === event.data.turn) return state
        return { ...state, turns: [...state.turns, { turn: event.data.turn, startTime: event.time, steps: [] }] }
      }
      case 'assistant/message': {
        if (state.last !== null
          && state.last.turn === event.data.turn
          && state.last.step === event.data.step) return state
        const usage = usageStep(event)
        if (usage === null) return state
        const step: TokenPricingStep = { ...usage, provider: state.route.provider, model: state.route.model }
        const last = state.turns.at(-1)
