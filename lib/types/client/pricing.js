 ...
 */
export function inWindow(start, end, nowMin) {
 ...
 */
export function inAnyPeakWindow(windows, timeZone, now) {
    const nowMin = minutesOfDay(now, timeZone);
    for (const window of windows) {
        if (inWindow(window.start, window.end, nowMin))
            return true;
    }
    return false;
}
 ...
 */
export function resolveEntry(entries, current) {
    if (current === null)
        return undefined;
    for (const entry of entries) {
        if (entry.model !== current.model)
            continue;
        if (entry.provider !== '' && entry.provider !== current.provider)
            continue;
        return entry;
    }
    return undefined;
 ...
 */
export function computeCost(entry, usage, now) {
 ...
 */
export function priceStep(entries, step) {
    const entry = resolveEntry(entries, { provider: step.provider, model: step.model });
    if (entry === undefined)
        return { step, entry: undefined, cost: undefined };
 ...
 */
export function aggregateByModel(projection, entries) {
    const steps = [];
 ...
 */
export function aggregateByTurn(projection, entries) {
    const turns = [];
    for (const turn of projection.turns) {
        const models = foldModels(turn.steps, entries);
        if (models.length > 0)
            turns.push({ turn: turn.turn, startTime: turn.startTime, models });
    }
    return turns;
}
/**
